...
def audio_analysis_result_right_pannel():
    return rx.vstack(
        rx.el.div(
            rx.text(
                audio_analysis_result_state.AudioAnalysisResultState.title,
                font_size="4.8em",
        ...
        rx.el.div(
            rx.markdown(
                audio_analysis_result_state.AudioAnalysisResultState.analysis_result,
                font_size="1.6em",
            ),
            line_height="2.8em",
        ),
...
def audio_analysis_result_buttons():
    return rx.hstack(
        ...
        #         "cursor": "pointer",
        #     },
        #     on_click=rx.download(url=audio_analysis_result_state.AudioAnalysisResultState.download_link),
        # ),
        rx.el.div(
            rx.el.div(
                "Start over",
...
def audio_analysis_result_options_item(text: str, index: int) -> rx.Component:
    return rx.hstack(
        ...
        style=audio_analysis_result_state.AudioAnalysisResultState.styles[index],
        cursor="pointer",
        on_click=audio_analysis_result_state.AudioAnalysisResultState.handleClick(
            index, text
        ),
        _hover={
            "background": rx.cond(
                audio_analysis_result_state.AudioAnalysisResultState.styles[index][
                    "background"
                ]
                != "black",
                "#00000008",
